...
describe('SaveManager', () => {
  it('starts with defaults and persists across instances', () => {
    ...
    expect(s1.data.ownedColors).toContain('#38bdf8');
    expect(s1.data.lastPlayedLevel).toBe(1);
    s1.data.coins = 777;
    s1.data.unlockedLevel = 4;
    s1.data.bestTimes[2] = 45.5;
    s1.data.ownedColors.push('#f87171');
    s1.data.lastPlayedLevel = 3;
    s1.flush();
    ...
    expect(s2.data.ownedColors).toContain('#f87171');
    expect(s2.data.lastPlayedLevel).toBe(3);
  });
