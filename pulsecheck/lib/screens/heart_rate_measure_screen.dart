...
/// The fingertip-camera pulse measurement flow. Saving a reading is the
/// only way heart rate data enters the app — there is no "type in your
/// heart rate" shortcut, since the whole point of this screen is the
...
class _HeartRateMeasureScreenState extends State<HeartRateMeasureScreen> with WidgetsBindingObserver {
  ...
  Future<void> _finishMeasuring() async {
    ...
    if (reading != null && reading.bpm != null && !_savedThisSession) {
      _savedThisSession = true;
      await _repository.addHeartRate(HeartRateReading(
        takenAt: DateTime.now(),
        bpm: reading.bpm!,
        rmssdMs: reading.rmssdMs,
      ));
      if (await _settings.healthSyncEnabled()) {
        await _healthKit.writeHeartRate(bpm: reading.bpm!, takenAt: DateTime.now());
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Measure heart rate')),
      body: SafeArea(
        child: Padding(
  ...
  Widget _buildBody() {
    switch (_state) {
      ...
      case _MeasureState.done:
        return _ResultView(
          reading: _reading,
          onMeasureAgain: () => setState(() => _state = _MeasureState.idle),
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            if (canFinishEarly) ElevatedButton(onPressed: onFinish, child: const Text('Done')),
          ],
        ),
      ],
    );
...
class _ResultView extends StatelessWidget {
  ...
  const _ResultView({required this.reading, required this.onMeasureAgain, required this.onDone});
    ...
    return Column(
      ...
      children: [
        if (bpm != null) ...[
          ...
          const Text('beats per minute — saved', style: TextStyle(fontSize: 20)),
          const SizedBox(height: 16),
          if (rmssd != null)
            Text(
          ...
          const SizedBox(height: 40),
          ElevatedButton(onPressed: onDone, child: const Text('Back to dashboard')),
          const SizedBox(height: 12),
          TextButton(onPressed: onMeasureAgain, child: const Text('Measure again')),
        ] else ...[
          ...
          const SizedBox(height: 40),
          ElevatedButton(onPressed: onMeasureAgain, child: const Text('Try again')),
        ],
      ],
    );
  }
