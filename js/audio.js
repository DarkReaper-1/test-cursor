/** Lightweight procedural audio. */
export function createAudio() {
  let ctx = null;
  let master = null;
  let started = false;
  let whoosh = null;
  let whooshGain = null;

  function ensure() {
    if (started) return;
    ctx = new (window.AudioContext || window.webkitAudioContext)();
    master = ctx.createGain();
    master.gain.value = 0.2;
    master.connect(ctx.destination);
    started = true;
    startAmbience();
    startWhoosh();
  }

  function startAmbience() {
    const osc = ctx.createOscillator();
    const g = ctx.createGain();
    osc.type = "sine";
    osc.frequency.value = 52;
    g.gain.value = 0.12;
    osc.connect(g); g.connect(master); osc.start();

    const osc2 = ctx.createOscillator();
    const g2 = ctx.createGain();
    osc2.type = "triangle";
    osc2.frequency.value = 104;
    g2.gain.value = 0.035;
    osc2.connect(g2); g2.connect(master); osc2.start();

    const lfo = ctx.createOscillator();
    const lfoG = ctx.createGain();
    lfo.frequency.value = 0.1;
    lfoG.gain.value = 0.025;
    lfo.connect(lfoG); lfoG.connect(g2.gain); lfo.start();
  }

  function startWhoosh() {
    // Filtered noise for speed whoosh
    const bufferSize = 2 * ctx.sampleRate;
    const buffer = ctx.createBuffer(1, bufferSize, ctx.sampleRate);
    const data = buffer.getChannelData(0);
    for (let i = 0; i < bufferSize; i++) data[i] = Math.random() * 2 - 1;
    whoosh = ctx.createBufferSource();
    whoosh.buffer = buffer;
    whoosh.loop = true;
    const filter = ctx.createBiquadFilter();
    filter.type = "bandpass";
    filter.frequency.value = 800;
    filter.Q.value = 0.7;
    whooshGain = ctx.createGain();
    whooshGain.gain.value = 0;
    whoosh.connect(filter);
    filter.connect(whooshGain);
    whooshGain.connect(master);
    whoosh.start();
  }

  function blip(freq, dur = 0.08, type = "square", vol = 0.2) {
    if (!started) return;
    const t = ctx.currentTime;
    const o = ctx.createOscillator();
    const g = ctx.createGain();
    o.type = type;
    o.frequency.setValueAtTime(freq, t);
    o.frequency.exponentialRampToValueAtTime(Math.max(40, freq * 0.7), t + dur);
    g.gain.setValueAtTime(vol, t);
    g.gain.exponentialRampToValueAtTime(0.001, t + dur);
    o.connect(g); g.connect(master);
    o.start(t); o.stop(t + dur + 0.02);
  }

  return {
    start: ensure,
    setSpeed(speed) {
      if (!whooshGain) return;
      const t = ctx.currentTime;
      const target = Math.min(0.12, Math.max(0, (speed - 15) * 0.004));
      whooshGain.gain.setTargetAtTime(target, t, 0.08);
    },
    webShoot() { blip(520, 0.09, "sawtooth", 0.13); blip(780, 0.06, "square", 0.06); },
    webRelease() { blip(200, 0.14, "triangle", 0.12); blip(140, 0.1, "sine", 0.08); },
    jump() { blip(240, 0.07, "square", 0.1); blip(360, 0.05, "triangle", 0.06); },
    doubleJump() { blip(320, 0.06, "square", 0.1); blip(480, 0.08, "sine", 0.08); },
    collect() { blip(700, 0.09, "sine", 0.16); blip(1040, 0.12, "sine", 0.11); },
    combo() { blip(560, 0.05, "square", 0.09); blip(840, 0.07, "sine", 0.06); },
    trick() { blip(440, 0.05, "square", 0.1); blip(660, 0.08, "triangle", 0.1); blip(880, 0.1, "sine", 0.08); },
    wallRun() { blip(180, 0.05, "sawtooth", 0.06); },
    fall() { blip(100, 0.4, "sawtooth", 0.15); },
  };
}
